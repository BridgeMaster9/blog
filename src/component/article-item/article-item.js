...
const ArticleItem = ({data: articleData, slug})=>{
  ...
  const confirm = ()=>{
    ...
    service.deleteArticle(slug, token)
      .then(()=>{
  ...
  const onLikeClick=()=>{
    ...
    if(token){
      if(data.favorited){
        service.removeFavorite(slug, token)
          .then((res)=>{
            setData(res.article)
          })
        ...
        service.addFavorite(slug, token).then((res)=>{
          setData(res.article)
        })
          ...
          (
            <div className={styles.buttons}>
              <Popconfirm
                ...
                <button className={styles.btn+' '+styles['btn-delete']} type='button'>Delete</button>
              </Popconfirm>
              <Link to={`/articles/${slug}/edit`}><button className={styles.btn+' '+styles['btn-create']} type='button'>Edit</button></Link>
            </div>
          ):null
        }
      </div>
