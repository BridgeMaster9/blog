...
function ArticleForm({ title, submit, data, isError, isSuccess }) {
  const {
    register,
    handleSubmit,
    control,
  ...
  return (
    <div className={styles.article}>
      <form onSubmit={handleSubmit(submit)}>
        ...
        {isError ? (
          <div className={`${styles.error} ${styles['error-alert']}`}>something happened wrong, try again</div>
        ) : null}
        ...
        <button className={`${styles.btn} ${styles['send-btn']}`} type="submit">
          Send
        </button>
      </form>
    </div>
